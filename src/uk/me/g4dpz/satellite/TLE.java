...
public class TLE implements Serializable {
	...
	public TLE(final String[] tle) throws IllegalArgumentException {
		...
		orbitnum = Integer.parseInt(tle[2].substring(63, 68).trim());
	 ...
	 */
	private void testArguments(final int lineCount, final String line) throws IllegalArgumentException {
		if (null == line) {
			throw new IllegalArgumentException(createIllegalArgumentMessage(lineCount, "was null"));
		}

		if (0 == line.length()) {
			throw new IllegalArgumentException(createIllegalArgumentMessage(lineCount, "was zero length"));
		}
	}
