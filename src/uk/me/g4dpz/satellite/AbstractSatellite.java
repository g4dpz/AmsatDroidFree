...
public abstract class AbstractSatellite implements Satellite, Serializable {
	...
	private void calculateLatLonAlt(final double time, final Vector4 positionVector, final SatPos satellitePosition) {
		...
		do {
			...
			satPos.setLatitude(Math.atan2(position.getZ() + EARTH_RADIUS_KM * c * e2 * Math.sin(phi), r));
	...
	@Override
	@Deprecated
	public synchronized void getPosition(final GroundStationPosition gsPos, final SatPos satellitePosition, final Date date) {
	 ...
	 */
	private static void calculateUserPosVel(final double time, final GroundStationPosition gsPos, final Vector4 obsPos,
			final Vector4 obsVel) {

		gsPos.setTheta(AbstractSatellite.mod2PI(AbstractSatellite.thetaGJD(time) + DEG2RAD * gsPos.getLongitude()));
		...
		final double achcp = (EARTH_RADIUS_KM * c + (gsPos.getHeightAMSL() / 1000.0)) * Math.cos(DEG2RAD * gsPos.getLatitude());
		obsPos.setXYZ(achcp * Math.cos(gsPos.getTheta()), achcp * Math.sin(gsPos.getTheta()),
				(EARTH_RADIUS_KM * sq + (gsPos.getHeightAMSL() / 1000.0)) * Math.sin(DEG2RAD * gsPos.getLatitude()));
	...
	private void calculateObs(final double julianUTC, final Vector4 positionVector, final Vector4 velocityVector,
		...
		range.setXYZ(positionVector.getX() - obsPos.getX(), positionVector.getY() - obsPos.getY(),
		...
		rgvel.setXYZ(velocityVector.getX() - obsVel.getX(), velocityVector.getY() - obsVel.getY(),
		...
		final double topS = sinLat * cosTheta * range.getX() + sinLat * sinTheta * range.getY() - cosLat * range.getZ();
		final double topE = -sinTheta * range.getX() + cosTheta * range.getY();
		final double topZ = cosLat * cosTheta * range.getX() + cosLat * sinTheta * range.getY() + sinLat * range.getZ();
	...
	public boolean willBeSeen(final GroundStationPosition qth) {
		...
		if (tle.getMeanmo() < 1e-8) {
			...
			final double sma = 331.25 * Math.exp(Math.log(1440.0 / tle.getMeanmo()) * (2.0 / 3.0));
			final double apogee = sma * (1.0 + tle.getEccn()) - EARTH_RADIUS_KM;

			return (Math.acos(EARTH_RADIUS_KM / (apogee + EARTH_RADIUS_KM)) + (lin * DEG2RAD)) > Math.abs(qth.getLatitude()
					* DEG2RAD);
		}
	...
	static class Vector4 extends Object implements Serializable {
		...
		public static final Vector4 scalarMultiply(final Vector4 vector, final double multiplier) {
			...
			return new Vector4(vector.w * Math.abs(multiplier), vector.x * multiplier, vector.y * multiplier, vector.z
					* multiplier);
		}
	 ...
	 * @param capu
	 */
	protected static void converge(final double[] temp, final double axn, final double ayn, final double capu) {
		...
		do {
			...
			temp[5] = axn * temp[8];
			temp[6] = ayn * temp[7];
			final double epw = (capu - temp[4] + temp[3] - temp[2]) / (1.0 - temp[5] - temp[6]) + temp[2];
	...
	private Vector4 calculateSunVector() {
		...
		final double m = radians(AbstractSatellite.modulus(358.47583 + AbstractSatellite.modulus(35999.04975 * solTime, 360.0)
				- (0.000150 + 0.0000033 * solTime) * AbstractSatellite.sqr(solTime), 360.0));
		final double l = radians(AbstractSatellite.modulus(279.69668 + AbstractSatellite.modulus(36000.76892 * solTime, 360.0)
				+ 0.0003025 * AbstractSatellite.sqr(solTime), 360.0));
		final double e = 0.01675104 - (0.0000418 + 0.000000126 * solTime) * solTime;
		final double c = radians((1.919460 - (0.004789 + 0.000014 * solTime) * solTime) * Math.sin(m)
				+ (0.020094 - 0.000100 * solTime) * Math.sin(2 * m) + 0.000293 * Math.sin(3 * m));
		...
		final double eps = radians(23.452294 - (0.0130125 + (0.00000164 - 0.000000503 * solTime) * solTime) * solTime + 0.00256
				* Math.cos(o));
	...
	protected void calculatePositionAndVelocity(final double rk, final double uk, final double xnodek, final double xinck,
